...
export default async function AuthErrorPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="grid min-h-screen place-items-center bg-[#f4f1ea] p-5"><div className="w-full max-w-md rounded-[30px] border border-[#1e2822]/10 bg-[#fffdf8] p-8 text-center shadow-xl"><span className="mx-auto grid size-14 place-items-center rounded-2xl bg-[#194d3a] text-white"><Sparkles className="size-6" /></span><h1 className="mt-6 font-serif text-4xl tracking-[-.04em]">Your email may already be confirmed</h1><p className="mt-4 leading-7 text-[#69766e]">This link may have expired or already been used. Sign in with the password you created to finish saving or joining your Plan. Your draft remains safe in this browser.</p><Link href={recoveryPath} className="mt-7 inline-flex rounded-full bg-[#d96545] px-6 py-3 text-sm font-bold text-white">Sign in to continue</Link><Link href="/" className="mt-4 block text-sm font-semibold text-[#657168]">Back to AgreeAway</Link></div></main>;
}
