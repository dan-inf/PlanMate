...
const joinGrowthDismissedKey = "agreeaway.join-growth-dismissed";
const legacyJoinGrowthDismissedKey = "planmate.join-growth-dismissed";

function joinGrowthWasDismissed() {
  const current = window.localStorage.getItem(joinGrowthDismissedKey);
  if (current === "1") return true;
  const legacy = window.localStorage.getItem(legacyJoinGrowthDismissedKey);
  if (legacy !== "1") return false;
  window.localStorage.setItem(joinGrowthDismissedKey, "1");
  window.localStorage.removeItem(legacyJoinGrowthDismissedKey);
  return true;
}
...
export function CollaborationWorkspace() {
  ...
  const [creationBalanceUnavailable, setCreationBalanceUnavailable] = useState(false);
  ...
  const loadPlan = useCallback(async (planId: string) => {
    ...
    setDays(nextDays);
    setMembers(memberRows.map((member) => ({ ...member, display_name: profileMap.get(member.user_id) || "AgreeAway member" })));
  ...
  const loadPlans = useCallback(async (preferredPlanId?: string) => {
    setCreationBalanceUnavailable(false);
    ...
    if (entitlementResult.error) {
      setCreationBalance(null);
      setCreationBalanceUnavailable(true);
    } else {
    ...
    if (invitationToken) {
      supabase.rpc("accept_plan_invitation", { invitation_token: invitationToken }).then(({ data, error: inviteError }) => {
        ...
        if (inviteError) setError(inviteError.message);
          ...
          setShowJoinGrowth(!joinGrowthWasDismissed());
  ...
  async function createDemoPlan() {
    ...
    const { data: createdPlan, error: planError } = await supabase.from("plans").insert({
      ...
      status: "active",
      source_prompt: "AgreeAway collaboration starter",
    }).select("id").single();
  ...
  async function proposeSavedEdit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !data.plan) throw new Error(data.error ?? "AgreeAway could not prepare that change.");
      ...
      setError(caughtError instanceof Error ? caughtError.message : "AgreeAway could not prepare that change.");
    ...
    <main className="min-h-screen bg-[#f4f1ea] text-[#1e2822]">
      <header className="border-b border-[#1e2822]/10 bg-[#fffdf8]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-[1440px] items-center justify-between px-5 sm:px-8">
          <Link href="/" className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-[14px] bg-[#194d3a] text-white"><Sparkles className="size-5" /></span><span className="text-xl font-semibold tracking-[-0.04em]">AgreeAway</span></Link>
      ...
      <div className="mx-auto grid max-w-[1440px] lg:grid-cols-[280px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 p-5 sm:p-8 lg:p-10 xl:p-12">
          ...
          {error ? <div role="alert" className="mb-6 flex flex-col gap-3 rounded-2xl bg-[#fff0eb] px-4 py-3 text-sm text-[#a4452f] sm:flex-row sm:items-center sm:justify-between"><span>{error}</span>{new URLSearchParams(window.location.search).get("save") === "generated" ? <button type="button" onClick={() => window.location.reload()} className="self-start rounded-full bg-[#a4452f] px-4 py-2 text-xs font-bold text-white sm:self-auto">Try again</button> : null}</div> : null}
          {!plan ? <PlanDashboard plans={plans} user={user} roles={planRoles} creationBalance={creationBalance} creationBalanceUnavailable={creationBalanceUnavailable} onRetryCreationStatus={() => void loadPlans()} onOpen={loadPlan} /> : <>
            ...
            {canEdit ? <section className="mt-8 rounded-[24px] border border-[#194d3a]/15 bg-[#e9f0ea] p-4 shadow-sm sm:p-5" aria-labelledby="saved-ask-agreeaway-heading"><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-3"><span className="mt-0.5 grid size-9 shrink-0 place-items-center rounded-xl bg-[#194d3a] text-[#f1c47b]"><Sparkles className="size-4" /></span><div><h2 id="saved-ask-agreeaway-heading" className="font-bold">Ask AgreeAway to change this plan</h2><p className="mt-1 text-sm leading-6 text-[#65736b]">Describe a broad change. You’ll review what changes before it is saved.</p></div></div>{latestEditId ? <button type="button" disabled={editBusy} onClick={undoSavedEdit} className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-white px-3 py-2 text-xs font-bold text-[#526159] disabled:opacity-50"><RotateCcw className="size-3.5" />Undo</button> : null}</div><form onSubmit={proposeSavedEdit} className="mt-4 flex flex-col gap-2 rounded-2xl border border-[#1e2822]/10 bg-white p-2 sm:flex-row sm:items-end"><label className="min-w-0 flex-1"><span className="sr-only">Describe a change to the saved plan</span><textarea value={savedEditInstruction} onChange={(event) => { setSavedEditInstruction(event.target.value); setProposedPlan(null); }} rows={2} maxLength={2000} placeholder="Make this cheaper, reduce driving, or move dinner later…" className="w-full resize-none bg-transparent px-3 py-2 text-sm leading-6 outline-none sm:text-base" /></label><button disabled={editBusy || savedEditInstruction.trim().length < 3} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-[#d96545] px-5 text-sm font-bold text-white disabled:opacity-40">{editBusy ? <LoaderCircle className="size-4 animate-spin" /> : <Send className="size-4" />}Review change</button></form>{proposedPlan ? <div className="mt-4 rounded-2xl border border-[#194d3a]/15 bg-[#fffdf8] p-4"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#d15d3e]">Proposed update</p><ul className="mt-3 space-y-1.5 text-sm text-[#59675f]">{proposedSummary.map((summary) => <li key={summary}>• {summary}</li>)}</ul><div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => setProposedPlan(null)} className="rounded-full px-4 py-2.5 text-sm font-semibold text-[#657168]">Keep current plan</button><button type="button" disabled={editBusy} onClick={applySavedEdit} className="rounded-full bg-[#194d3a] px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">Apply and save</button></div></div> : null}</section> : null}
...
function UnifiedPlanItemCard({ item, userId, members, votes, comments, locked, onVote, onComment }: { item: Item; userId: string; members: Member[]; votes: Vote[]; comments: Comment[]; locked: boolean; onVote: (id: string, value: -1 | 1) => void; onComment: (id: string, body: string) => void }) {
  ...
  const nameFor = (id: string) => members.find((member) => member.user_id === id)?.display_name ?? "AgreeAway member";
...
function PlanDashboard({ plans, user, roles, creationBalance, creationBalanceUnavailable, onRetryCreationStatus, onOpen }: { plans: PlanRow[]; user: User; roles: Record<string, Member["role"]>; creationBalance: number | null; creationBalanceUnavailable: boolean; onRetryCreationStatus: () => void; onOpen: (id: string) => void }) {
